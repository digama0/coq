...
let match_pattern (type c) (p : c p) : t -> ast * c =
  ...
  match p with
  | PKEYWORD s -> (function KEYWORD s' when seq s s' -> (Akeyword s, s') | NUMBER n when seq s (NumTok.Unsigned.sprint n) -> (Akeyword s, s) | _ -> err ())
  | PIDENT None -> (function IDENT s' -> (Aident s', s') | _ -> err ())
  | PIDENT (Some s) -> (function (IDENT s' | KEYWORD s') when seq s s' -> (Aident s, s') | _ -> err ())
  | PPATTERNIDENT None -> (function PATTERNIDENT s -> (Apatternident s, s) | _ -> err ())
  | PPATTERNIDENT (Some s) -> (function PATTERNIDENT s' when seq s s' -> (Apatternident s, s') | _ -> err ())
